...
// if (argc==3)	{input=argv[3]);}
	...
	string project_root_path = "/home/levcsi/IPCV_Madrid/AVSA/Lab_1/AVSA2020results"; //SET THIS DIRECTORY according to your project
	...
	int NumCat = sizeof(dataset_cat)/sizeof(dataset_cat[0]); //number of categories (have faith ... it works! ;) ... each string size is 32 -at least for the current values-)
	...
	for (int c=0; c<NumCat; c++ )
	{
		...
		int NumSeq = sizeof(baseline_seq)/sizeof(baseline_seq[0]);  //number of sequences per category ((have faith ... it works! ;) ... each string size is 32 -at least for the current values-)
...
//			namedWindow("ForeGround Mask"); // final fgmask
			...
			double tau = 0; // to set ... //Lab1.1.1-3
